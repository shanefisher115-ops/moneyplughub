  ...
  public static async runAutomation(
    ...
    try {
      ...
      if (automationId === 'auto_daily_balance_check') {
        ...
        const monthlyEarnings = db.prepare(`SELECT * FROM earnings_snapshots WHERE user_id = ? AND window = 'monthly'`).get(userId) as Record<string, unknown> | undefined;
        // verified
      } else {
      ...
      const endedAt = new Date().toISOString();
      const errorMsg = err instanceof Error ? err.message : 'Automation execution failed.';
      const failedLog: CanonicalRunLog = {
        runId,
        automationId,
        status: 'failure',
