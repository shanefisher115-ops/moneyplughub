...
export class StarterOrchestrator {
  ...
  public static async executeCommand(
    ...
    payload?: { preferredSlug?: string; [key: string]: unknown }
  ): Promise<{
    success: boolean;
    data?: unknown;
    error?: string;
    ...
    try {
      ...
      console.error('StarterOrchestrator error:', errorObj);
      this.handleFailure(userId, errorObj.message || 'Execution failure');
  ...
  public static getState(userId: string): OrchestratorState {
    ...
    `).get(userId) as { status?: OrchestratorStatus; consecutive_failures?: number; last_run_at?: string; degraded_reason?: string } | undefined;
