...
import { fetchTop100Creators, fetchTopSyndicates, leaderboardWsManager } from '../leaderboard/ws';
...
/**
 * Leaderboard Rankings (Top 100 Creators, Earnings Tiers, Syndicate Rankings & Badges)
 */
router.get('/leaderboard', (req: AuthenticatedRequest, res: Response) => {
  const currentUserId = req.user!.id;

  const creators = fetchTop100Creators(currentUserId);
  const syndicates = fetchTopSyndicates(currentUserId);
